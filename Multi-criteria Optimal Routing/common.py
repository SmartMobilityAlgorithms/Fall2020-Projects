...
class Node:
    ...
    def expand(self):
        children = [Node(graph = self.G, osmid = child, distance = self.node[child][0]['travel_time'], parent = self) \
...
The function calculated the weight of the route based on length
"""

def cost_dist(G, route):
    weight = 0
...
"""
Given an iterable with nodes ids and the networkx graph
The function calculated the weight of the route based on travel time
"""

def cost_time(G, route):
    weight = 0
    for u, v in zip(route, route[1:]):
...
"""
Given an iterable with nodes ids and the networkx graph
The function calculated the cost of a route with constraints

@param       G: graph
@param   route: a list of nodes that describes a route
@param      ks: a list of weight coefficients
@param    keys: a list of attributes
@param  orders: a list of power orders for each attribute
"""

def cost_with_constraints(G, route, ks, keys, orders):
    weight = 0
    for u, v in zip(route, route[1:]):
        edge = G[u][v][0]
        for k, key, order in zip(ks, keys, orders):
            weight += k * pow(edge[key], order)
    return weight
